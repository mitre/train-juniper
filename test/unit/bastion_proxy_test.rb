...
describe 'BastionProxy module' do
  ...
  describe 'configure_bastion_proxy' do
    it 'should configure bastion proxy with custom port' do
      ...
      _(ENV['SSH_ASKPASS']).must_match(%r{/ssh_askpass.*\.sh$})
      _(ENV['SSH_ASKPASS_REQUIRE']).must_equal('force')
    end
  ...
  describe 'setup_bastion_password_auth' do
    it 'should set up SSH_ASKPASS when bastion password provided' do
      # Save original env
      original_askpass = ENV['SSH_ASKPASS']
      original_require = ENV['SSH_ASKPASS_REQUIRE']
    ...
    it 'should use device password if bastion password not provided' do
      ...
      script_content = File.read(ENV['SSH_ASKPASS'])
    ...
    it 'should not set up SSH_ASKPASS when no password available' do
      ...
      if original_askpass.nil?
        _(ENV['SSH_ASKPASS']).must_be_nil
      else
        _(ENV['SSH_ASKPASS']).must_equal(original_askpass)
      end
      
      if original_require.nil?
        _(ENV['SSH_ASKPASS_REQUIRE']).must_be_nil
      else
        _(ENV['SSH_ASKPASS_REQUIRE']).must_equal(original_require)
      end
    end
  end
