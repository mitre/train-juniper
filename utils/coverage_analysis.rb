...
def parse_options
  ...
  OptionParser.new do |opts|
    ...
    opts.on("-f", "--format FORMAT", [:human, :json, :markdown], 
...
def format_json_output(files, overall, total_lines, total_covered, incomplete_files, complete_files, impact_files)
  result = {
    ...
    }.sort_by { |f| f[:percentage] },
    recommendations: impact_files.map { |file, data|
      {
        file: file,
...
def format_markdown_output(files, overall, total_lines, total_covered, incomplete_files, complete_files, impact_files)
  ...
  output << ""
  if overall >= 90
    output << "✅ **Coverage exceeds 90% threshold - excellent!**"
  elsif overall >= 80
    output << "✅ **Coverage meets 80% industry standard**"
  else
    output << "⚠️ **Coverage below 80% industry standard**"
  end
